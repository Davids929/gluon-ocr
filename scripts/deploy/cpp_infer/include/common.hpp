...
inline void Normalize(cv::Mat *rgb_image){
    ...
    for (int h = 0; h < rgb_image->rows; h++) {
        ...
        rgb_image->at<cv::Vec3f>(h, w)[0] =
            (rgb_image->at<cv::Vec3f>(h, w)[0] - mean[0]) * scale[0];
        rgb_image->at<cv::Vec3f>(h, w)[1] =
            (rgb_image->at<cv::Vec3f>(h, w)[1] - mean[1]) * scale[1];
        rgb_image->at<cv::Vec3f>(h, w)[2] =
            (rgb_image->at<cv::Vec3f>(h, w)[2] - mean[2]) * scale[2];
        }
    }
};

// Load data from CV BGR image
inline NDArray AsData(cv::Mat bgr_image, Context ctx = Context::cpu(), bool norm=true) {
...
inline void LoadCheckpoint(const std::string model_path, const std::string params_path,
                           Symbol* symbol, std::map<std::string, NDArray>* arg_params,
                           std::map<std::string, NDArray>* aux_params,
                           Context ctx = Context::cpu()) {
    // load symbol from JSON
    Symbol new_symbol = Symbol::Load(model_path);
    // load parameters
    std::map<std::string, NDArray> params = NDArray::LoadToMap(params_path);
